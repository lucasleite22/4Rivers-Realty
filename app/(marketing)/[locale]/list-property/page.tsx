...
const PROPERTY_TYPE_KEYS = ['HORSE_FARM', 'RANCH', 'RESIDENTIAL', 'COMMERCIAL', 'LAND'] as const

// API stores this as free text in the admin (English-only) lead notes, so the
// submitted value must stay a readable English label regardless of UI locale.
const PROPERTY_TYPE_EN_LABELS: Record<(typeof PROPERTY_TYPE_KEYS)[number], string> = {
            ...
            {workflowSteps.map((step, i) => (
              <div key={step.key} className="relative">
                ...
                <div className="relative z-10">
                  ...
                  <p className="font-barlow text-xs font-bold text-gray-300 tracking-widest mb-2">{t('step', { number: String(i + 1).padStart(2, '0') })}</p>
                  <h3 className="font-cormorant font-bold text-xl text-navy mb-2">{t(`steps.${step.key}.title`)}</h3>
                  <p className="font-barlow text-sm text-gray-500 leading-relaxed">{t(`steps.${step.key}.description`)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 md:p-10">
            ...
            {status === 'success' && (
              <div className="flex items-center gap-3 bg-green-50 border border-green-200 text-green-800 rounded-lg px-5 py-4 mb-8">
                ...
                <div>
                  <p className="font-barlow font-semibold text-sm">{t('successTitle')}</p>
                  <p className="font-barlow text-sm">{t('successSubtitle')}</p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Contact */}
              <div>
                <p className="font-barlow text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">{t('sections.yourInfo')}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">
                      {t('fields.fullName')} <span className="text-brand-blue">*</span>
                    </label>
                  ...
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.email')} <span className="text-brand-blue">*</span></label>
                    <input required type="email" value={form.email} onChange={(e) => set('email', e.target.value)}
                      placeholder={t('placeholders.email')}
                      className="w-full px-4 py-3 rounded-lg border border-gray-200 font-barlow text-sm text-navy placeholder-gray-400 focus:outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" />
                  </div>
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.phone')} <span className="text-brand-blue">*</span></label>
              ...
              {/* Property details */}
              <div>
                <p className="font-barlow text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">{t('sections.propertyDetails')}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.propertyType')} <span className="text-brand-blue">*</span></label>
                    <select required value={form.propertyType} onChange={(e) => set('propertyType', e.target.value)}
                      ...
                      <option value="">{t('placeholders.propertyType')}</option>
                      {PROPERTY_TYPE_KEYS.map((key) => (
                        <option key={key} value={PROPERTY_TYPE_EN_LABELS[key]}>{tTypes(key)}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.county')}</label>
                    <input value={form.county} onChange={(e) => set('county', e.target.value)}
                      placeholder={t('placeholders.county')}
                      className="w-full px-4 py-3 rounded-lg border border-gray-200 font-barlow text-sm text-navy placeholder-gray-400 focus:outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.address')} <span className="text-brand-blue">*</span></label>
                    <input required value={form.address} onChange={(e) => set('address', e.target.value)}
                      placeholder={t('placeholders.address')}
                      className="w-full px-4 py-3 rounded-lg border border-gray-200 font-barlow text-sm text-navy placeholder-gray-400 focus:outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" />
                  </div>
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.acreage')}</label>
                    <input type="number" min="0" value={form.acreage} onChange={(e) => set('acreage', e.target.value)}
                      placeholder={t('placeholders.acreage')}
                      className="w-full px-4 py-3 rounded-lg border border-gray-200 font-barlow text-sm text-navy placeholder-gray-400 focus:outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" />
                  </div>
                  <div>
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.askingPrice')}</label>
                  ...
                  <div className="sm:col-span-2">
                    <label className="font-barlow text-sm font-semibold text-navy block mb-1.5">{t('fields.notes')}</label>
              ...
              {/* Photo upload */}
              <div>
                <p className="font-barlow text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">{t('sections.propertyPhotos')}</p>
                <p className="font-barlow text-xs text-gray-400 mb-3">{t('photos.hint')}</p>
                ...
                {previews.length < 8 && (
                  ...
                  >
                    ...
                    <p className="font-barlow text-sm text-gray-400">
                      <span className="font-semibold text-brand-blue">{t('photos.click')}</span> {t('photos.drag')}
                    </p>
                    <input
                      ref={fileInputRef}
                      type="file"
              ...
              >
                {status === 'loading' ? t('submitting') : (<>{t('submit')} <ArrowRight className="w-4 h-4" /></>)}
              </button>
